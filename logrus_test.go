...
import (
	...
	. "github.com/ividernvi/ivilog/internal/testutils"
)
...
func TestReportCallerWhenConfigured(t *testing.T) {
	...
	LogAndAssertJSON(t, func(log *Logger) {
		...
		assert.Equal(t, "info", fields["level"])
		assert.Equal(t,
			"github.com/ividernvi/ivilog_test.TestReportCallerWhenConfigured.func3", fields[FieldKeyFunc])
	})
...
func TestNestedLoggingReportsCorrectCaller(t *testing.T) {
	...
	assert.Equal(t, "eating raw fish", fields["context"])
	assert.Equal(t,
		"github.com/ividernvi/ivilog_test.TestNestedLoggingReportsCorrectCaller", fields["func"])
	cwd, err := os.Getwd()
